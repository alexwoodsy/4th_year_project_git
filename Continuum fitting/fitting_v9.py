...
def contfitv9(specsample,zlim,stonlim,gcredshift,showplot,showerror):
    ...
    #fitting:
        ...
        winpeak = np.array([])
        forestwinnum, forestpct = 8, 0.2 #forest number must be even
        ...
        if redshift < zlim and showerror:
            ...
            while step <= speclen:
                if step <= lyalphaind:
                    ...
                    #         startind = step - int(pw/2) + findmed(flux[(step-int(pw/2)):ind])
        ...
        #pad interval with start/end value to allign correctly
            ...
            contfit = intpol(wlen)
            #smooth fit
            smoothwin = 2*int(lyalphaind/forestwinnum)+1 # has to be odd hence forestwinnum must be even
            #print(smoothwin)
            forrange = np.arange(0,(speclen))
