...
=======
rend = 1000
>>>>>>> c20566289e93d95f4cae4c84a06271c58b26bcfc
###############------carla selection ------##################
    ...
    if len(specselect) == 0:#catched cluster
        ...
        for ind in specselect:
            ...
            if leecheck == 0 and chooselee == True: #check lee fit is in sample
                ...
                if gc_qso_sep < rstart or gc_qso_sep > rend:
                    ...
                    qso_sn = np.append(qso_sn, stonforest)
                    qso_gcz = np.append(qso_gcz,gcredshift)
                    qso_gcname.append(gcname)

                    if showerror == True:
                        print('adding '+spec+' S/N = '+ str(stonforest) +'and z = '+ str(redshift) +' to stack.')
        ...
        if errortot == len(stackstatus):
            ...
            #save name of carla for saving
            carlastacked.append(gcname)
...
fig2, ax = plt.subplots(2,1,num='multi-carla stack - uncombined')
...
fig3, ax = plt.subplots(2,1,num='('+str(carlasucces)+'/'+str(carlanumber)+ ') stacked clusters')
...
if saveoutput == True:
    #save stacked wlen, flux in lamba/v_rel for opeing again for med/median. + stack info in metadata extension
    ...
    #ext2 = carlatargets in stack
    carlastackedcol = fits.Column(name='CARLA', array = carlastacked, format='20A')
    ...
    hdul = fits.HDUList([primary, stacksavedata, carlastackedsavedata, qsostackedsavedata])

    outname = 'stacking/figures/Stacking data/' + runsavename+ '.fits'
