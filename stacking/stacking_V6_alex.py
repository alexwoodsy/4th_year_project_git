...
for carlaselect in range(0,2): #all change to - matchlen
    ...
    plt.figure('uncombined carla stacks')
    plt.plot(dsrange, dsnormspewcstack,label = match[carlaselect] + ' stack of '+str(stacktot) + ' QSO')
    plt.xlabel(r'$\lambda$ ($\mathrm{\AA}$)')
    plt.ylabel(r'$F$ $(10^{-17}$ ergs $s^{-1}cm^{-2}\mathrm{\AA}^{-1})$')
    plt.legend()
...
#
pltrange = np.arange(0,int(len(multistack)*0.6))
meanstack = (multistack[pltrange]/specstacktot)
normstack = meanstack/np.max(abs(meanstack))



plt.figure('multi-carla stack with '+ str(specstacktot) +' spectra')
plt.subplot(2, 1, 1)
plt.plot(dsrange[pltrange], meanstack[pltrange])
plt.plot(np.array([(1215.67),(1215.67)]),np.array([np.min(meanstack),np.max(meanstack)]),'--')
string = r' Ly$\alpha$ absorbtion'
plt.text(1215.67, 5, string)
plt.xlabel(r'$\lambda$ ($\mathrm{\AA}$)')
plt.ylabel(r'$<F>$ $(10^{-17}$ ergs $s^{-1}cm^{-2}\mathrm{\AA}^{-1})$')

plt.subplot(2, 1, 2)
plt.plot(dsrange[pltrange], normstack[pltrange])
plt.plot(np.array([(1215.67),(1215.67)]),np.array([np.min(normstack),np.max(normstack)]),'--')
string = match[carlaselect] + ' z = '+ str(gcredshift) + r' Ly$\alpha$ absorbtion'
plt.text(1215.67, 1, string)
plt.xlabel(r'$\lambda$ ($\mathrm{\AA}$)')
plt.ylabel(r'$norm <F>$ $(10^{-17}$ ergs $s^{-1}cm^{-2}\mathrm{\AA}^{-1})$')
